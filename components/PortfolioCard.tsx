...
const PortfolioCard = ({portfolio}: PortfolioCardProps) => {
  ...
  async function searchPortfolio(){
    if(chainId == 137 && portfolio.contract_address == "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"){
      portfolio.tokenName= "Matic";
      portfolio.tokenSymbol= "MATIC";
      portfolio.tokenLogo = "https://tokens.1inch.io/0x7d1afa7b718fb893db30a3abc0cfc608aacfebb0.png";
    }
    if(!portfolio.tokenName){
      ...
      if(result){
        const matchedToken = tokenList.find((item) => item.symbol === result.data);
  ...
  return (
    loading ? <div></div> :  
    <div className="w-[90%] text-xs sm:text-md md:text-base md:w-1/3 h-[10%] bg-gray-700 bg-opacity-80 rounded-xl p-4 flex justify-between items-center drop-shadow-xl font-semibold">
      <div className="w-[70%] h-full flex items-center gap-4">
        ...
        <div className="flex-col h-full w-full justify-center flex gap-2">
          ...
          <div className="w-fit h-full flex gap-2 text-gray-400 items-center">
            ...
            <p>{portfolio.price_to_usd >= 1000 ? `$${Math.ceil(portfolio.price_to_usd)}` : `$${Math.ceil(portfolio.price_to_usd * 1000) / 1000}`}</p> :
            <p> {`<$0.001`} </p> 
            }
            <p
            ...
            >
              {portfolio.roi >= 0 ? "+" : ""}{Math.ceil(portfolio.roi * 10000) / 100}%
            </p>
          </div>
        </div>
      </div>
      <div className="flex-col h-full w-[30%] justify-center flex gap-2 items-end p-1">
        <p>${Math.ceil(portfolio.value_usd * 100) / 100}</p>
        <p className="text-gray-400">
          {portfolio.amount >= 1000 ? `${Math.ceil(portfolio.amount)}` : `${Math.ceil(portfolio.amount * 10000) / 10000}`} {portfolio.tokenSymbol}
        </p>
      </div>
    </div>
  );
